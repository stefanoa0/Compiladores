   ...
   // }
            ...
            for(vetInd=0;vetInd<100;vetInd++){
                if(vet[vetInd] != -1){
                    if(queue[vet[vetInd]].lexeme == "TK_if" || queue[vet[vetInd]].lexeme == "TK_while"){
                        ...
                        while(queue[vet[vetInd]].lexeme != "TK_achave"){
                            ...
                            if(queue[vet[vetInd]].lexeme == "TK_ident"){
                                if(queue[vet[vetInd]].value[0] == '-'){
                                    fprintf(fpObj, "lit,0,0\n");
                                    fprintf(fpObj, "sto,0,%d\n",memInd);
                                    fprintf(fpObj, "lod,0,%d\n",memInd);
                                }
                                else{
                        ...
                        if(fluxo){
                            if(strcmp(fluxo->elem, "w") == 0) {
                                    fprintf(fpObj, "jpc,0,$\n");
                                    pushInt(&lineJPC,linhaObj++);
                                    //printf("%d", linhaObj);
                                    //getchar();
                                    //pushInt(&lineJPC,5+linhaObj++);
                            }
                            else{
                                    fprintf(fpObj, "jpc,0,$\n");
                                    //pushInt(&lineJPC,linhaObj++);
                    ...
                    if(queue[vet[vetInd]].lexeme == "TK_igual"){
                        fprintf(fpObj, "lit,0,%s\n", queue[vet[++vetInd]].value);
                        fprintf(fpObj, "sto,0,%d\n",memInd);
                        fprintf(fpObj, "lod,0,%d\n",memInd);
                        memInd++;linhaObj+=3;
            ...
            input = fopen("Obj.txt", "r");

            //lê o arquivo char a char e já subistitui
            while(!feof(input))
            {
                fscanf(input, "%c", &temp);

                if(temp == FLAG){
                    substituto = popInt(&lineJPC);
                    fprintf(output, "%d", linhaObj);
                }
                else
